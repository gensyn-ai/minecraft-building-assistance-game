...
def test_randomly_permute_block_types():
    for teleportation in [False, True]:
        for inf_blocks in [True]:
            for flat_actions in [False, True]:
                horizon = 100
                episodes = 2
                env = MbagEnv(
                    {
                ...
                for t in range(horizon):
                    ...
                    mask = MbagActionDistribution.get_mask_flat(env.config, obs_batch)[
                        0
                    ]
                ...
                if flat_actions:
                    ...
                    batch[SampleBatch.ACTIONS] = tuple(
                        np.array(
                            [action[action_part] for action in all_actions] * episodes
                        )
                        for action_part in range(3)
                    )

                new_batch = randomly_permute_block_types(
                    batch, flat_actions=flat_actions, env_config=env.config
                )

                # Episode should be permuted so it doesn't match the original
                # (except with low probability).
                assert np.any(
                    new_batch[SampleBatch.OBS][0] != batch[SampleBatch.OBS][0]
                )
                ...
                for episode_index in range(episodes):
                    ...
                    (obs,) = env.set_state(
                        mbag_obs_to_state(initial_obs, player_index=0)
                    )
                    for t in range(horizon):
                        i = episode_index * horizon + t
                        np.testing.assert_equal(
                            obs[0], new_batch[SampleBatch.OBS][0][i]
                        )
                        np.testing.assert_equal(
                            obs[1], new_batch[SampleBatch.OBS][1][i]
                        )
                        np.testing.assert_equal(
                            obs[2], new_batch[SampleBatch.OBS][2][i]
                        )
                        if flat_actions:
                            action = tuple(
                                action_map[new_batch[SampleBatch.ACTIONS][i]]
                            )
                        else:
                            action = cast(
                                MbagActionTuple,
